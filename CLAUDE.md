...
npm run pdf:sample [theme] [cover] [size] [perPage] [lang] [sticker]   # write korice/strane/nalepnice PDFs to tmp/ from fixture data; the word "sticker" anywhere in the line puts the numbers on the picture instead of the backing
...
- **Sticker numbers are never stored** — a slot's number is its reading-order position, recomputed on every read (`packages/shared/src/numbering.ts`). Its `position` *is* stored, and names a grid cell rather than a running index: a turned sticker takes two cells and leaves a hole, so `renumber` renumbers pages and stickers but never touches a slot's position.
- **Album size and stickers-per-page are locked at creation** — changing either would add/destroy slots in an album that may already have photos. Sticker orientation is chosen beside them (it decides the cell shape, so it decides which grids exist) but is only a *starting point*: see below.
- **Any one sticker can be turned, and a turned one takes the room of two** — a sticker is a physical 50 x 70 mm rectangle that is never resized, only turned, so the only place a 70 mm-wide one fits on a grid of 50 mm cells is across two of them. `slotSpanOf` in `geometry.ts` is the whole rule — which cells a slot covers and where its outline sits inside them — and both renderers call it instead of `slotRect`. `POST /slots/:id/turn` swallows the neighbouring cell (deleting that sticker; the editor asks first when it holds a photo) and standing the sticker back up hands the cell out again as a fresh empty one. It turns along the row in an album of standing stickers and down the column in one of lying stickers, taking the cell *behind* it when it is at the end of the run.
- **A lying sticker is printed lying inside an upright cell** — the sticker sheet keeps one grid, one cut pitch, one calibration bar and one sheet count, whatever the album mixes. `Panel.turned` in `pdf/canvas.ts` pushes the quarter turn onto the PDF's own graphics state, so `drawSticker` needs to know nothing about it. Whoever cuts the sticker out turns it.
- **A sticker's number is printed on its backing paper by default, and that is a choice at print time** — the number gets the sticker cut out and matched to a slot, then peels away with the liner instead of sitting on the photo forever. So a sticker sheet is normally two PDF pages: stickers on the front, one bare numeral centred in each cell on the back, mirrored top-to-bottom by `stickerBackRect` in `geometry.ts` because the whole job prints with a short-edge flip and this is its one upright sheet. Not every printer will feed sticker paper twice, so the print dialog offers the Panini way too — `numbers=sticker` puts the badge back in the corner of the picture and the second side disappears. It is a `PrintOptions` on the request (`?numbers=`), never a column: the same album prints both ways. `duplexFor` in `printing.ts` is the one place that turns it into a number of sides, which the badges, the shop note, the print-shop sheet's own URL and each PDF's `Subject` then say for themselves; `ctx.numbers` carries it to all three builders, because the cover's "how to stick them in" has to tell a child which side to look at.
...
- **A photo can be turned inside its sticker, and that is a different thing** — `crop.rotate` is a quarter turn stored with the rest of the crop, for a picture that came out of a phone lying down. `photoPlacement` in `imaging.ts` fits a turned photo to the window *lying on its side* and rotates it back onto it, which is what keeps a sideways picture filling the sticker instead of arriving with two white wedges; `components/FramedPhoto.tsx` and `Panel.image` are the two renderers of that one answer.
...
- **A photo too small for the paper is marked, never refused** — `printDpi` in `imaging.ts` reads `photoPlacement` backwards: the pixels divided by the millimetres they are stretched across. It has to be asked through the placement, because that is where the pixels go — a wide photo in an upright sticker prints from its short side alone, and zoom spends what is left on less of the picture, and neither of those shows in the file's own dimensions. Below `MIN_PRINT_DPI` the editor marks the sticker on the page and says why in the dialog that can fix it (`.slot__warn`, `.lowres`); the cover is judged against the album's *real* page from `layoutFor`, not `REF_PAGE`, so one photo passes on a small album's A5 cover and fails on a large one's A4. Nothing is ever blocked. The floor is 200 dpi rather than the 300 we aim for on purpose: our own upload cap lands an ordinary 16:9 phone photo at 299, and a warning a child sees on every second picture is one they stop reading. The caps in `config.ts` are the other half of that bargain — they must leave a good photo above the floor, or the warning is about our downsampling rather than the child's camera.
...
- **A found picture is never fetched by address** — a result carries a `pick`, the address signed by this process with a per-process key and good for fifteen minutes, and the fetch route will only go and get a picture for a `pick` it signed itself. `apps/server/src/remotefetch.ts` is the guard behind that: https only, resolved addresses checked against the private ranges *and the socket pinned to the address that was checked* (resolving twice is the rebinding hole), redirects followed by hand through the same checks, byte cap enforced during the read. A 401 or 403 is asked once more with no referer and nothing else changed — naming the picture's own host is what gets it served by a site that checks for hotlinking, and naming anything at all is what gets it refused by a CDN sitting on a different host from the page, and there is no telling which is which in advance. Everything from there on is the ordinary upload path — same `storeImage`, same stripping, same `live.publish`.
- **A dragged picture is the one address a client may name** — the commonest way a photo arrives is a drag out of an image search in the next window, and the file the browser synthesises from that drag is the *results page's* thumbnail: a few hundred pixels, fine on glass, a mosaic on paper. The drag also carries `text/uri-list` (the link the thumbnail sat inside, which a search engine fills with the original's address) and `text/html` (the element's own markup, `srcset` included). `betterPictureUrl` in `packages/shared/src/drops.ts` reads those two strings and answers whether there is a bigger original worth going after; `apps/web/src/drop.ts` is the half that needs a DataTransfer, and `POST /images/from-drop` fetches it. This is a deliberate exception to the bullet above and the only one: the address is not signed, because no signature could cover a URL that came off the child's own screen. What replaces it is `remotefetch.ts` unchanged (now load-bearing alone), the rate limit a found picture pays, an album token to spend it against, and `DROP_FETCH=off`. Nothing can lose a picture — every failure falls back to uploading the dropped file, which is what always used to happen, so the client needs no knowledge of whether the server is willing.
...
```text
packages/shared/src/
  geometry.ts     paper sizes, reference page, every grid position, and which
                  cells one sticker covers once it has been turned
  imaging.ts      where a photo sits in its window, turn included
  ...
  drops.ts        which picture a drag is really pointing at, as string work
apps/server/src/
...
apps/web/src/
  components/PageSheet, CoverSheet  mirror what the PDF prints, at editor zoom
                  ...
                  FramedPhoto       a photo filling a window, turned as the child left it
  drop.ts         a drop's two sources, and which one is worth trying first
